...
describe("SimpleBaseAccessible", function () {
  ...
  beforeEach(async function () {
    ...
    SimpleBaseAccessible = await ethers.getContractFactory(
      "SimpleBaseAccessible"
    );
    simpleBaseAccessible = await SimpleBaseAccessible.deploy(
      owner.address,
      superAdmin.address
    );
  ...
  it("should initialize with the correct roles", async function () {
    expect(
      await simpleBaseAccessible.hasRole(
        await simpleBaseAccessible.DEFAULT_ADMIN_ROLE(),
        owner.address
      )
    ).to.be.true;
    expect(
      await simpleBaseAccessible.hasRole(
        await simpleBaseAccessible._getSuperAdminRole(),
        superAdmin.address
      )
    ).to.be.true;
  });
  ...
  it("should allow the super admin to add and remove an admin", async function () {
    ...
    expect(
      await simpleBaseAccessible.hasRole(
        await simpleBaseAccessible._getAdminRole(),
        addr1.address
      )
    ).to.be.true;

    await simpleBaseAccessible
      .connect(superAdmin)
      .testRemoveAdmin(addr1.address);
    expect(
      await simpleBaseAccessible.hasRole(
        await simpleBaseAccessible._getAdminRole(),
        addr1.address
  ...
  it("should allow the owner to change the owner and super admin", async function () {
    ...
    expect(
      await simpleBaseAccessible.hasRole(
        await simpleBaseAccessible._getOwnerRole(),
        addr1.address
      )
    ).to.be.true;
  ...
  it("should allow the default admin to set a role admin", async function () {
    ...
    await simpleBaseAccessible
      .connect(owner)
      .testSetRoleAdmin(role, newAdminRole);
    expect(await simpleBaseAccessible.getRoleAdmin(role)).to.equal(
      newAdminRole
    );
  });
  ...
  it("should check if an address has a specific role", async function () {
    ...
    expect(await simpleBaseAccessible.hasSpecificRole(role, addr1.address)).to
      .be.true;
    expect(await simpleBaseAccessible.hasSpecificRole(role, addr2.address)).to
      .be.false;
  });
});
